...
- **🧠 Jarvis/O9 autonomous supervisor (fork)** — `jarvis-auto` routes Claude Code through a self-maintaining strict-zero-cost pool with live provider discovery, bounded Claude tool probes, OpenCode keyless onboarding, quality + provider-diversity ranking, lifecycle/cooldown state, and a Free Pool observatory. Paid or cost-unproven routes never enter the strict child. → [Provider Runtime State](docs/architecture/PROVIDER_RUNTIME_STATE.md) · [Runbook](docs/runbooks/JARVIS_AUTONOMOUS_FREE_POOL.md)
