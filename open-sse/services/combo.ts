...
// O9-F3.3P1-D5: raw FCC soft-preference fact per candidate — pure, DB-free,
// hard-gated inside the function itself (see fccRankingSignal.ts docblock).
// No parallel ranking pipeline: this is the same D0 signal function, fed by
// the same D1/D2 facts the D4 visibility gate already uses.
import { resolveFccPreferenceSignal } from "./fccRankingSignal.ts";
import {
  ...
  const candidates = await Promise.all(
    fingerprintExpandedTargets.map(async (target) => {
      ...
      return {
        ...
        // O9-F3.3P1-D5: raw FCC soft-preference fact, already hard-gated
        // (executable && claudeCodeEligible && FCC-proven-compatible) inside
        // resolveFccPreferenceSignal. Route-scoping (coding-only) and the
        // default-0 weight both live in scoring.ts — this is purely the fact.
        fccPreference: resolveFccPreferenceSignal(provider, model),
      };
    })
  );
